...
describe('NutritionScreen — la feuille se ferme après le travail, pas avant', () => {
  ...
  it('compte la série de saisie, et la garde en remontant la semaine', async () => {
    const today = toLogicalDate()
    const yesterday = addDays(today, -1)
    // Hier et avant-hier saisis, aujourd'hui pas encore : la série vaut 2 et ne
    // repart pas de zéro sous prétexte que le petit-déj n'est pas noté.
    for (const [id, date] of [
      ['log-hier', yesterday],
      ['log-avant-hier', addDays(today, -2)],
    ]) {
    ...
    await waitFor(() => expect(screen.getByText(plain(t.nutrition.streakLabel(2)))).toBeTruthy())
    ...
    expect(screen.getByText(plain(t.nutrition.streakLabel(2)))).toBeTruthy()
  })
